"""
书法GAN快速验证训练脚本（激进模式）
64x64 + batch 16 + 15 epochs → 约40分钟跑完
"""
...
CONFIG = {
    ...
    "cache_dir"    : str(ALGO_DIR / "data" / ".skeleton_cache_fast128"),
    "ckpt_dir"     : str(PROJECT_DIR / "checkpoints"),
    "image_size"   : 128,      # 保持128（模型结构固定4次上采样）
    "style_dim"    : 128,
    "content_dim"  : 256,
    "batch_size"   : 16,       # 加速：更大batch
    "num_epochs"   : 15,       # 加速：15 epochs
    "lr_g"         : 2e-4,
    "lr_d"         : 4e-4,
    "save_every"   : 5,
...
class Trainer:
    def __init__(self, cfg):
        ...
        sd = cfg["style_dim"]; cd = cfg["content_dim"]
    ...
    def train_epoch(self, loader):
        ...
        for batch in loader:
            ...
            loss_d, _ = self.loss_fn.compute_discriminator_loss(
            ...
            loss_g, _ = self.loss_fn.compute_generator_loss(
    ...
    def save(self, epoch, ckpt_dir, tag=""):
        ...
        name = f"checkpoint_fast64_epoch_{epoch:03d}{tag}.pth"
        torch.save({
            "epoch": epoch,
...
def main():
    cfg = CONFIG
    print("=" * 60)
    print("  春意墨生 — 快速验证训练 (128px/16batch/15ep)")
    print(f"  数据目录  : {cfg['data_root']}")
    print(f"  图片尺寸  : {cfg['image_size']}x{cfg['image_size']}")
    print(f"  Batch     : {cfg['batch_size']}   Epoch: {cfg['num_epochs']}")
    print(f"  设备      : {cfg['device']}")
    print("=" * 60)

    print("\n[1/3] 加载数据集...")
    ds = CalligraphyDS(cfg["data_root"], cfg["cache_dir"], cfg["image_size"],
                        cfg["aug_factor"], STYLE_MAP)
    if len(ds) == 0:
    ...
    with open(os.path.join(final_dir, "style_map.json"), "w", encoding="utf-8") as f:
        json.dump({"style_map": STYLE_MAP, "image_size": cfg["image_size"],
                   "style_dim": cfg["style_dim"], "content_dim": cfg["content_dim"]},
                  f, ensure_ascii=False, indent=2)

    with open(os.path.join(cfg["ckpt_dir"], "train_log_fast64.json"), "w") as f:
        json.dump(log, f, indent=2)

    print(f"\n[DONE] 快速验证训练完成! 最佳 G Loss: {best_g:.4f}")
    print(f"   模型: {final_dir}/checkpoint_fast64_epoch_{cfg['num_epochs']:03d}.pth")
